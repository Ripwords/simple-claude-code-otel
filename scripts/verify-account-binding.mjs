...
const impostor = rewriteAccount(metrics, '00000000-dead-4000-8000-000000000000', 'someone.else@example.com')
...
const rowsBefore = (await call('GET', `/api/stats/summary?from=${new Date(Date.now() - 30 * 864e5).toISOString()}&to=${new Date(Date.now() + 864e5).toISOString()}`)).json[0]
...
const rowsAfter = (await call('GET', `/api/stats/summary?from=${new Date(Date.now() - 30 * 864e5).toISOString()}&to=${new Date(Date.now() + 864e5).toISOString()}`)).json[0]
